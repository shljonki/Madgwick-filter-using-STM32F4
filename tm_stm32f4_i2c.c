...
uint8_t ID_ReadNackkojivraca0akofaila(I2C_TypeDef* I2Cx){
	uint8_t data;
...
uint8_t TM_I2C_Read(I2C_TypeDef* I2Cx, uint8_t address, uint8_t reg) {
	...
	if(address==0x1A) //poseban slucaj za provjerit INT_STATUS_1 koji je po defaultu 1
	received_data = ID_ReadNackkojivraca0akofaila(I2Cx);
...
int16_t TM_I2C_Start(I2C_TypeDef* I2Cx, uint8_t address, uint8_t direction, uint8_t ack) {
	...
	/* SDA se spušta dole dok je SCL gore*/
	...
	while (!(I2Cx->SR1 & I2C_SR1_SB)){
	 if (--TM_I2C_Timeout == 0x00) {
			return 1; //nisam dobio ack bit natrag
		}
	}
	...
	if (direction == I2C_TRANSMITTER_MODE) {
		/* Send address (7 bit) with zero last bit (+1 bit) */
		/*read je logic high, WRITE je logic low*/
		I2Cx->DR = address & ~I2C_OAR1_ADD0; //and invertirani I2C_OAR1_ADD0 oznacava transmit, sto je nula
	...
	if (direction == I2C_RECEIVER_MODE) {
		...
		/*READ je logic high, write je logic low*/
		I2Cx->DR = address | I2C_OAR1_ADD0; //oznaci receive
	...
	return 0; //dobio sam ack bit natrag
}
